...
import { selectedTrackAtom } from "@/store/select-track-store";
...
function BtnBar() {
  ...
  const { isLoading, data } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_SITE_URL}/api/kkbox/info?column=artist_id&value=${selectedArtistsStr}`,
      );
  ...
  return (
    <div className="absolute bottom-0 z-10 flex items-center justify-center gap-4">
      <div className="flex h-10 w-32 select-none items-center justify-center rounded-md border border-primary text-primary">
